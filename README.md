...
客户端请求HTML -----> 服务端返回一个HTML ------> 客户端下载css，JS等文件 -------> 客户端加载JS并完成初始化 -------> JS代码可以执行，醒后端发起ajax请求返回数据 --------> 等待后台返回数据 --------> react-dom从无到有完整的，把数据渲染为页面

服务端渲染：

客户端请求一个HTML -------> 服务端请求数据（内网请求快）------> 服务端初始渲染 -------> 服务端返回有正确内容的页面 -------> 客户端请求并下载JS／css的文件 -------> 客户端加载JS文件完成初始化 --------> react-dom把剩下的一部分内容完成渲染
...
- 最主要的问题是关于fetch请求的位置，在客户端渲染的时候，请求放在组件生命周期的componentDidMount里面，但是在服务端渲染的时候，组件只会执行到componentWillMount，而不执行组件生命周期的其他阶段，但是请求还是不能放在componentWillMount，因为renderToString是同步的，而请求时异步的，如果在componentWillMount里面渲染数据以后执行setState不会出发任何刷新，所以服务端渲染的时候请求不应该放在组件的生命周期里，而应该在node端请求好数据，renderToString之前先把数据通过props传递给组件进行渲染，然后再ssR返回渲染好的组件。

### 2018年3月5日

这次的新工程使用的是webpack4， 关于用到的一些新的特性的坑和自己的理解记录一下
